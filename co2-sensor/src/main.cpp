...
void writeFile(SDLib::SDClass &fs, const char *path, const char *message);
void printTime();
...
void appendFile(SDLib::SDClass &fs, const char *path, const char *message);
...
void loadStateFile() {
    fs::File file = SPIFFS.open(STATE_FILENAME, "r");
...
void saveStateFile(struct state *oldstate, struct state *state) {
    ...
    fs::File f = SPIFFS.open(STATE_FILENAME, "w");
    f.print(
...
void initSD() {
    ...
    SDLib::File file = SD.open("/data.txt");
    if (!file) {
...
void loadConfigData() {
    fs::File file = SPIFFS.open(CONFIG_FILENAME, "r");
...
void saveConfigData() {
    fs::File file = SPIFFS.open(CONFIG_FILENAME, "w");
...
void writeFile(SDLib::SDClass &fs, const char *path, const char *message) {
    ...
    SDLib::File file = fs.open(path, FILE_WRITE);
    if (!file) {
...
void appendFile(SDLib::SDClass &fs, const char *path, const char *message) {
    ...
    SDLib::File file = fs.open(path, 'a');
    if (!file) {
