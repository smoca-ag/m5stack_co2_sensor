        ...
        if (sensors[i].id == sensor_id) {
            for (u8_t j = 0; j < sensors[i].measurement_types_len; j++) {
                if (sensors[i].measurement_types[j] == measurement_type) {
                    // struct measurement_row row = { .sensor_id = sensor_id, .measurement_type = measurement_type };
                    cell_instance->reference.u32 = measurement_type;
                    // cell_instance->reference_len = sizeof(row);
    ...
    struct snmp_node_instance* cell_instance)
{
    ...
    for (i = 0; i < LWIP_ARRAYSIZE(sensors); i++) {
        for (j = 0; j < sensors[i].measurement_types_len; j++) {
            ...
            /* check generated OID: is it a candidate for the next one? */
            // struct measurement_row row = { .sensor_id = test_oid[0], .measurement_type = test_oid[1] };
            
            snmp_next_oid_check(&state, test_oid,
                LWIP_ARRAYSIZE(measurement_table_oid_ranges), test_oid[1] /*&row*/);
        }
    }
    /* did we find a next one? */
    if (state.status == SNMP_NEXT_OID_STATUS_SUCCESS) {
        snmp_oid_assign(row_oid, state.next_oid, state.next_oid_len);
        /* store sensor index for subsequent operations (get/test/set) */
        cell_instance->reference.u32 = state.reference;
        // cell_instance->reference_len = sizeof((struct measurement_row*)state.reference);

        // struct measurement_row *row = cell_instance->reference.ptr;
        // u32_t id = row->sensor_id;
        // u32_t type = row->measurement_type;
        // char sid[1];
        // char stype[1];
    ...
    struct snmp_node_instance* cell_instance, void* value)
{
    ...
    u32_t measurement_type = cell_instance->reference.u32;
    ...
    case 2: {
        ...
        *v = get_measurement(measurement_type);
        value_len = sizeof(v);
    } break;
    default: {
        LWIP_DEBUGF(
...
static s16_t int_get_value(struct snmp_node_instance* instance, void* value)
{
    s32_t* v = (s32_t*)value;
